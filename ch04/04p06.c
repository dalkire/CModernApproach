/*
 * Ch04 Pr06 | C Programming, A Modern Approach
 *
 * European countries use a 13-digit code, known as a European Article
 * Number (EAN) instead of the 12-digit Universal Product Code (UPC)
 * found in North America. Each EAN ends with a check digit, just as
 * a UPC does. The technique for calculating the check digit is also
 * similar:
 ...
 * which has an EAN of 8691484260008. The first sum is 6 + 1 + 8 + 2 +
 * 0 + 0 = 17, and the second sum is 8 + 9 + 4 + 4 + 6 + 0 = 31.
